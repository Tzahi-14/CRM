...
const addClients = async function (array) {
    ...
    for (let i = 0; i <= newArr.length - 1; i++) {
        const fullName = newArr[i].name.split(" ")
        const firstName = fullName[0]
        const lastName = fullName[1]
        let clientData = await sequelize.query(`INSERT INTO client VALUES(null,'${firstName}','${lastName}','${newArr[i].email}','${newArr[i].firstContact.replace('T', ' ').replace('Z', '')}',${newArr[i].sold},${newArr[i].emailType},${newArr[i].owner},${newArr[i].country})`)
